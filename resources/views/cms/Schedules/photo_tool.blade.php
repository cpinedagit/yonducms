...
<div class="modal modal-custom fade" id="fileModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static">
...
//    $(document).ready(function () {
    $('#insert').on('click', function () {
	  ...
	  var schedCount = {{ $schedCount }};
	  var checkedCount = $("input:checkbox[name=cbfiles]:checked").length;
	  var sumCount = schedCount + checkedCount;
        $("input:checkbox[name=cbfiles]:checked").each(function () {
            selected.push($(this).val());
        });
        var id = $('#scheduleBannerId').val();
        console.log(selected);
	  if(sumCount <= 5){
		$.ajax({
		    method: 'put',
		    url: '{!! route("cms.scheduler.saveScheduleBannerImage") !!}',
		    headers: {
		...
		alert('You cannot exceed the maximum of 5 images per schedule!');
		return false;
	  }
